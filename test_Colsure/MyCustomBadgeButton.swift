...
//  test_Colsure
//
//  Created by Jingnan Zhang on 16/5/27.
//  Copyright © 2016年 Jingnan Zhang. All rights reserved.

//  完全OK的 封装；  和qq的效果不一致；长按手势比这要好点
...
class MyCustomBadgeButton: UIButton, CAAnimationDelegate {
    ...
    var isRemoveShapleyer:Bool {  // 是否移除了shapelayer
        get{
...
//    }
    ...
    func addInsideCircleInSuperView(_ superView: UIView){
        ...
        // 加一个和自己默认大小一样的内圆 btn
        insideCircle.frame = CGRect(x: self.frame.origin.x, y: self.frame.origin.y, width: MyCustomBadgeButtonH, height: MyCustomBadgeButtonH)
    ...
    // MARK: 点击事件, 只有直接点击按钮或按钮拖动距离太大时才会调
    func clickAction(_ button: MyCustomBadgeButton) {
        
//        let startPoint = CGPointMake(self.frame.origin.x, self.frame.origin.y)
//        let stopPoint = CGPointMake(self_Frame.origin.x, self_Frame.origin.y)
//        
//        let dis = calculateDistanceFromPoint(fromPoint: startPoint, toPoint: stopPoint)
        if !isClickBtn { // 非直接点击按钮而触发此法
    ...
    // MARK: 缩放、shake动画
    private func handleGeture(_ pan: UIPanGestureRecognizer){
    ...
    // MARK: Change 时的动画
    private func doChangeAnimate(_ aniView:MyCustomBadgeButton, insideCircle:UIButton){
    ...
    private func doEndAnimate(_ fromView:MyCustomBadgeButton, toView:UIButton){
...
//            // 执行点击事件的方法
    ...
    private func doDismissAnimate(_ atPoint:CGPoint){
        ...
        aniImageView.bounds = CGRect(x: 0, y: 0, width: MyCustomBadgeButtonH * 2,  height: MyCustomBadgeButtonH * 2)
